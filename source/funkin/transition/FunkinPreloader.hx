...
import funkin.transition.VFDOverlay;
...
class FunkinPreloader extends FlxBasePreloader {
     static final BASE_WIDTH:Float  = 1280;   // The logo image width at the base resolution. Scaled up/down appropriately as needed.

     static final BAR_PADDING:Float = 20;     // Margin at the sides and bottom, around the loading bar.
     static final BAR_HEIGHT:Int    = 12;

     static final LOGO_FADE_TIME:Float = 2.5; // Logo takes this long (in seconds) to fade in.
     var ratio:Float = 0;                     // Ratio between window size and BASE_WIDTH
     ...
     private var completeTime:Float = -1; // The timestamp when the other steps completed and the `Finishing up` step started.
     ...
     override function create():Void {
          super.create(); // Nothing happens in the base preloader.

          Lib.current.stage.color = Constants.COLOR_PRELOADER_BG; // Background color.
                         ...
                         */
                         ...
                         if (cachingGraphicsComplete && elapsedCachingGraphics >= Constants.PRELOADER_MIN_STAGE_TIME) {
                              ...
                              if (cachingGraphicsPercent < (elapsedCachingGraphics / Constants.PRELOADER_MIN_STAGE_TIME)) {
                                   return cachingGraphicsPercent; // Return real progress if it's lower.
                              } else {
                                   return elapsedCachingGraphics / Constants.PRELOADER_MIN_STAGE_TIME; // Return simulated progress if it's higher.
                              }
                         }
                    } else {
