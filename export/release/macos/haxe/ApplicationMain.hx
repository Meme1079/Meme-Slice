...
class ApplicationMain
{
	...
	public static function create(config):Void
	{
		...
		app.meta["build"] = "19";
		app.meta["company"] = "Meme1079";
		app.meta["file"] = "PsychEngine";
		app.meta["name"] = "Friday Night Funkin': Psych Engine - Meme-Slice";
		...
		var attributes:lime.ui.WindowAttributes = {
			...
			parameters: {},
			resizable: true,
			title: "Friday Night Funkin': Psych Engine - Meme-Slice",
			width: 1280,
			x: null,
			y: null,
		};
		...
		#elseif air
		app.window.title = "Friday Night Funkin': Psych Engine - Meme-Slice";
		#else
	...
	macro public static function getPreloader()
	{
		...
		var type = Context.getType("funkin.transition.FunkinPreloader");
			...
			case TInst(classType, _):
				...
				while (searchTypes != null)
				{
					if (searchTypes.pack.length == 2 && searchTypes.pack[0] == "openfl" && searchTypes.pack[1] == "display" && searchTypes.name == "Preloader")
					{
						return macro
						{
							new funkin.transition.FunkinPreloader();
						};
					}
		...
		return macro
		{
			new openfl.display.Preloader(new funkin.transition.FunkinPreloader());
		}
